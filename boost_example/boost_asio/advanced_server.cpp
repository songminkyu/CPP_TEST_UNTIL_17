...
//파일의 확장자를 기반으로 합당한 MIME 형식을 반환합니다.
...
// Append an HTTP rel-path to a local filesystem path.   HTTP rel-path를 로컬 파일 시스템 경로에 추가하십시오.
// The returned path is normalized for the platform.     반환 된 경로는 플랫폼에 대해 정규화됩니다.
std::string
path_cat(
...
/*
 ...
 이 함수는 주어진 요청에 대한 HTTP 응답을 생성합니다. 
 응답 객체의 유형은 요청의 내용에 따라 다르므로 인터페이스에서는 
 호출자가 응답을 수신하기 위해 일반 람다를 전달해야합니다.
 */
...
public:
	...
	void on_timer(boost::system::error_code ec)
	{
		...
		{
			std::cout << "void on_timer(boost::system::error_code ec)" << std::endl;
...
int main(int argc, char* argv[])
{
	...
	*/
	...
	std::string const doc_root = R"(d:\Release\html\)";//doc_root 는 무조건 파일 지정이 아닌 폴더 경로 지정 해줘야 함. 이거때문에 존나 고생햇네.
	auto const threads = std::max<std::size_t>(1, std::atoi("5"));
